...
def upload(request):
    form = None
    campaign_name = handler.getCampaignNames()
    ...
    campaign_amount = handler.getCampaignLength()
    sitetraffic_amount = handler.getSiteTrafficLength()

    context = {
        "campaign_nameJSON": campaign_name.to_json(),
        "campaign_amount": campaign_amount,
        "sitetraffic_amount": sitetraffic_amount,
        "form": form
    }
...
def clearsuccess(request):
    clear_campaign = request.GET.get("campaign") == 'true'
    clear_sitetraffic = request.GET.get("sitetraffic") == 'true'
    old_campaign_amount = handler.getCampaignLength()
    old_sitetraffic_amount = handler.getSiteTrafficLength()
    if clear_campaign:
        handler.clearCampaign()
    if clear_sitetraffic:
        handler.clearSiteTraffic()
    new_campaign_amount = handler.getCampaignLength()
    new_sitetraffic_amount = handler.getSiteTrafficLength()
    delete_campaign_amount = new_campaign_amount - old_campaign_amount
    delete_sitetraffic_amount = new_sitetraffic_amount - old_sitetraffic_amount
    context = {
        "old_campaign_amount": old_campaign_amount,
        "old_sitetraffic_amount": old_sitetraffic_amount,
        "new_campaign_amount": new_campaign_amount,
        "new_sitetraffic_amount": new_sitetraffic_amount,
        "delete_campaign_amount": delete_campaign_amount,
        "delete_sitetraffic_amount": delete_sitetraffic_amount
    }
    return render(request, 'upload/clear-success.html', context)
