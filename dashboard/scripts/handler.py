...
def colCheck(col, type):  # already plus 2 as for year and period
    if col == 12 and type == "GS":
        return True
    elif col == 22 and type == "FB":
        return True
    elif col == 46 and type == "LM":
        return True
    elif col == 17 and type == "LP":
        return True
    elif col == 19 and type == "GC":
        return True
    else:
        return False
...
def insertCampaign(df):
    ...
    for index, row in df.iterrows():
        cursor.execute("INSERT INTO dbo.Campaign (date, period, year, cid, name, pid, spending, reach, impression, engagement, objective, video_view, landing_page_view, product_view, add_to_cart, purchase, dateadded) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                       row.date, row.period, row.year, row.cid, row.cname, row.pid, row.spending, row.reach, row.impression, row.engagement, row.objective, row.video_view, row.landing_page_view, row.product_view, row.add_to_cart, row.purchase, row.dateadded)
...
def insertSiteTraffic(df):
    ...
    for index, row in df.iterrows():
        cursor.execute("INSERT INTO dbo.SiteTraffic (sid, pid, period, year, uri, all_user, new_user, order_count, revenue, dateadded) values (?,?,?,?,?,?,?,?,?,?)",
                       row.sid, row.pid, row.period, row.year, row.uri, row.all_user, row.new_user, row.order_count, row.revenue, row.dateadded)
...
def clearSiteTraffic():
    cnxn = pyodbc.connect('DRIVER={SQL Server};SERVER='+HOST +
                          ';DATABASE='+DATABASE+';UID='+USER+';PWD=' + PASSWORD)
    cursor = cnxn.cursor()
    cursor.execute("DELETE FROM dbo.SiteTraffic")
    cnxn.commit()
    cursor.close()
