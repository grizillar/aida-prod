...
fillAnnualLineGraph();
var currChart = createDateLineGraph();

function createDateLineGraph(target = "engagement") {
	if (by == "date") {
		const data_l1_d = {
		...
		document.getElementById("summary-per-title").innerHTML = `กราฟผลรวม ${capitalizeFirstLetter(target)} ต่อเดือนตั้งแต่ ${monthof(earliestMonth[0])}, ${earliestMonth[1]} ถึง ${monthof(lastestMonth[0])}, ${lastestMonth[1]}`;
...
function fillAnnualLineGraph() {
	if (by == "period") {
		for (let i = 0; i < multiple_selector.length; i++) {
			const data_l1_p = {
				labels: interpretPeriod(PERIODINYEAR),
				datasets: formDatasetWithPeriod(multiple_selector[i]),
			};
			...
			document.getElementById(`summary-per-title-${multiple_selector[i]}`).innerHTML = `กราฟผลรวม ${capitalizeFirstLetter(multiple_selector[i])} รายปี แบ่งตามพีเรียด`;
			new Chart(document.getElementById(`line-1-${multiple_selector[i]}`), config_l1_p);
		}
	}
}
...
function minorDropdown(target) {
	currChart.destroy();
	currChart = createDateLineGraph(target);
}
