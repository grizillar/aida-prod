		...
		if (compare) {
			...
			if (isYearSame_2) {
				if (isPeriodSame_2) {
					...
					range_text_2 = `${period_2[0]} ปี ${year_2[0]}`;
				} else {
					time_text_2 = `ตั้งแต่พีเรียดที่ ${period_2[0]} ถึงพีเรียดที่ ${period_2[1]} ปี ${year_2[0]}`;
					range_text_2 = `${period_2[0]} - ${period_2[1]} ปี ${year_2[0]}`;
				}
			} else {
				time_text_2 = `ตั้งแต่พีเรียดที่ ${period_2[0]} ปี ${year_2[0]} ถึงพีเรียดที่ ${period_2[1]} ปี ${year_2[1]}`;
				range_text_2 = `${period_2[0]} ปี ${year_2[0]} - ${period_2[1]} ปี ${year_2[1]}`;
			}

			var time_text_3 = `เปรียบเทียบระหว่างพีเรียดที่ ${range_text} กับพีเรียดที่ ${range_text_2}`;
...
function showAnnualGraph() {
	if (by == "period") {
		document.getElementById("date-annual-row").classList.add("hidden");
		document.getElementById("annually-line-graph").classList.remove("hidden");
		fillAnnualLineGraph("annually-line-graph");
	}
}

function addSummaryTotal() {
	var summary_tbody = document.getElementById("summary-total");
	var tr_format = `
	<tr class="total-row">
		<td id="a-7-0"></td>
		<td id="a-7-1">Total</td>
		<td id="a-7-2">${formatNumber(totalSummary.spending[0])}</td>
		<td id="a-7-3">${formatNumber(totalSummary.reach[0])}</td>
		<td id="a-7-4">${formatNumber(totalSummary.impression[0])}</td>
		<td id="a-7-5">${formatNumber(totalSummary.engagement[0])}</td>
		<td id="a-7-6">${formatNumber(totalSummary.user[0])}</td>
		<td id="a-7-7">${formatNumber(totalSummary.new_user[0])}</td>
		<td id="a-7-8">${formatNumber(totalSummary.order[0])}</td>
		<td id="a-7-9">${formatNumber(totalSummary.revenue[0])}</td>
	</tr>`;
	summary_tbody.innerHTML += tr_format;
	if (compare) {
		var summary_2_tbody = document.getElementById("summary-2-total");
		var tr_format_2 = `
		<tr class="total-row">
			<td id="c-7-0"></td>
			<td id="c-7-1">Total</td>
			<td id="c-7-2">${formatNumber(totalSummary_2.spending[0])}</td>
			<td id="c-7-3">${formatNumber(totalSummary_2.reach[0])}</td>
			<td id="c-7-4">${formatNumber(totalSummary_2.impression[0])}</td>
			<td id="c-7-5">${formatNumber(totalSummary_2.engagement[0])}</td>
			<td id="c-7-6">${formatNumber(totalSummary_2.user[0])}</td>
			<td id="c-7-7">${formatNumber(totalSummary_2.new_user[0])}</td>
			<td id="c-7-8">${formatNumber(totalSummary_2.order[0])}</td>
			<td id="c-7-9">${formatNumber(totalSummary_2.revenue[0])}</td>
		</tr>`;
		summary_2_tbody.innerHTML += tr_format_2;

		var summary_compare_tbody = document.getElementById("summary-compare-table-total");
		var tr_format_c = `
		<tr class="total-row">
			<td id="d-7-0"></td>
...
function unfocusPlatform() {
	let platform_x = [];
	for (let i = 0; i < platform.length; i++) {
		platform_x.push(parseInt(platform[i]) - 1);
	}

	for (let x = 0; x < 7; x++) {
		for (let y = 0; y < 10; y++) {
			if (!platform_x.includes(x)) {
				var cellcode = `a-${x}-${y}`;
				var cell = document.getElementById(cellcode);
...
function formatNumber(num) {
	...
	if (num_str.includes(".")) {
		var dec_count = num_str.slice(num_str.indexOf(".") + 1).length;
		if (dec_count == 1) {
			num_str = num_str + "0";
		}
		var number_components = num_str.split(".");
		number_components[0] = number_components[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
		num_str = `${number_components[0]}.${number_components[1]}`;
	} else {
		num_str = num_str.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
	}
	return num_str;
}
		...
		<div class="col-3 d-flex">
				<div class="card">
					<div class="card-body donut-chart-card">
						<p class="graph-title">แผนภูมิเปรียบเทียบผลรวม<br />${capitalizeFirstLetter(multiple_selector[i])} ของแต่ละแพลตฟอร์ม</p>
...
function fillAnnualLineGraph(target) {
	if (by == "period") {
		var target_div = document.getElementById(target);
		for (let i = 0; i < multiple_selector.length; i++) {
			target_div.innerHTML += `
			<div class="row">
				<div class="col-12">
					<div class="card">
						<div class="card-body">
							<p class="graph-title" id="summary-per-title-${multiple_selector[i]}"></p>
							<div id="g-l1-${i}">
								<canvas id="line-1-${multiple_selector[i]}"></canvas>
							</div>
						</div>
					</div>
				</div>
...
function querySubmit() {
	...
	if (currently_interval == "date-tab") {
		...
		if (startdate_date && enddate_date && startdate_date > enddate_date) {
			throwError(E8, true);
			return;
		}
	}
	if (currently_interval == "period-tab") {
		...
		if (year_1_s > year_1_e || (year_1_s == year_1_e && period_1_s > period_1_e)) {
			throwError(E9, true);
			return;
		}
		if (compare_toggle) {
			...
			if (year_2_s > year_2_e || (year_2_s == year_2_e && period_2_s > period_2_e)) {
				throwError(E10, true);
				return;
			}
		}
